...
function App() {
  ...
  useEffect(() => {
    if (tipo === 'ingreso' && prodId) {
      ...
      if (p) {
        ...
        if (!descripcion) setDescripcion(`Venta de ${p.nombre}`);
        setMonto(p.precio * cantidad);
      }
    } else { setPrecioUnitario(0); }
  }, [prodId, cantidad, tipo]);

  useEffect(() => {
    setBusquedaVenta(''); setProdId(''); setDescripcion(''); setMonto('');
  }, [tipo]);
  ...
  const guardarMovimiento = async (e) => {
    ...
    const respuesta = await fetch('http://localhost:3000/api/finanzas', {
      ...
      body: JSON.stringify({ 
        descripcion, monto: parseFloat(monto), tipo: tipoParaBD, cantidad: parseInt(cantidad), 
        producto_id: prodFinal, fecha: new Date().toISOString(), usuario: session.user.email 
      })
    });
    ...
    <div className="min-h-screen p-6 transition-all" style={{ backgroundImage: fondo ? `url(${fondo})` : 'none', backgroundColor: '#f3f4f6', backgroundSize: 'cover', backgroundAttachment: 'fixed' }}>
      <div className={`max-w-7xl mx-auto p-6 rounded-xl shadow-2xl ${fondo ? 'bg-black/90 text-white' : 'bg-white text-gray-800'}`}>
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-6 border-b border-gray-700 pb-4 gap-4">
          <div>
              <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-500">CODICIA ERP</h1>
          ...
          <div className="flex flex-col items-end gap-1"><input type="text" placeholder="Fondo URL..." className="bg-transparent border-b text-xs w-32" onChange={e => setFondo(e.target.value)} /><button onClick={cerrarSesion} className="text-red-500 text-xs font-bold hover:underline">CERRAR TURNO</button></div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className={`lg:col-span-5 p-6 rounded-xl border ${fondo ? 'bg-white/5 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
            ...
            <form onSubmit={guardarMovimiento} className="space-y-4">
              <div className="flex bg-gray-200 rounded p-1 gap-1">
                ...
                <button type="button" onClick={() => setTipo('gasto_stock')} className={`flex-1 py-1 rounded text-sm font-bold ${tipo === 'gasto_stock' ? 'bg-white shadow text-blue-700' : 'text-gray-500'}`}>STOCK</button>
              </div>
              ...
              {tipo === 'gasto' ? (
                ...
                <div className="animate-fadeIn space-y-3">
                        ...
                        {productosVenta.length === 0 ? (<div className="p-3 text-center text-sm text-gray-400">❌ No encontrado</div>) : (
                            productosVenta.map(p => (
                                <div key={p.id} onClick={() => setProdId(p.id)} className={`p-3 cursor-pointer border-b text-sm flex justify-between items-center transition-colors ${prodId == p.id ? 'bg-blue-100 border-l-4 border-blue-500 text-blue-800' : 'hover:bg-gray-50 text-gray-700'}`}>
                                    <div><span className="font-bold block">{p.nombre}</span><span className="text-xs opacity-70">{tipo === 'ingreso' ? `$${p.precio.toLocaleString()}` : `Stock: ${p.stock}`}</span></div>
                    ...
                    <div className="flex gap-2 mt-2">
                        <div className="w-24"><label className="text-[10px] font-bold">CANT</label><input type="number" className="w-full p-2 rounded text-black border text-center font-bold" value={cantidad} onChange={e => setCantidad(e.target.value)} /></div>
              ...
              <button type="submit" className={`w-full text-white font-bold py-4 rounded shadow-lg active:scale-95 transition-all ${tipo === 'ingreso' ? 'bg-green-600' : (tipo === 'gasto_stock' ? 'bg-blue-600' : 'bg-red-600')}`}>
                {tipo === 'ingreso' ? 'COBRAR' : (tipo === 'gasto_stock' ? 'COMPRAR STOCK' : 'REGISTRAR')}
              </button>
            </form>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className={`p-4 rounded-xl border flex-1 ${fondo ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
                {/* CABECERA CON BUSCADOR DE INVENTARIO Y PAPELERA */}
                <div className="flex flex-col gap-2 mb-2">
                    <div className="flex justify-between items-center">
                        <h3 className="font-bold">📦 Inventario {verEliminados ? '(PAPELERA)' : ''}</h3>
                    ...
                    <div className="flex gap-2 items-center">
                        <input type="text" placeholder="🔍 Buscar en inventario..." className="w-full p-2 text-sm rounded border bg-gray-50 text-black" value={busquedaInventario} onChange={e => setBusquedaInventario(e.target.value)} />
                ...
                <div className="overflow-auto max-h-56">
                    <table className="w-full text-sm"><thead className="opacity-60 text-xs border-b"><tr><th>Prod</th><th>$$</th><th>Stock</th><th>Acciones</th></tr></thead>
            ...
            <div className={`p-4 rounded-xl border flex-1 ${fondo ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'}`}>
                <h3 className="font-bold mb-2">📅 Movimientos (Auditoría)</h3>
                <div className="overflow-auto max-h-56">
                    <table className="w-full text-sm">
                        <thead className="opacity-60 text-xs border-b"><tr><th>Hora</th><th>User</th><th>Desc</th><th>Monto</th></tr></thead>
                        <tbody>
